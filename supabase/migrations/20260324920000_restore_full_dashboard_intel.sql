-- Fix: restore full get_vendor_dashboard_intel function.
--
-- The sentiment_enrichment migration (20260323100000) overwrote this
-- function with a stripped-down version that lost:
--   1. Entity resolution (vendor_entities / canonical name)
--   2. ai_insight + is_emerging on feature gaps
--   3. Supporting dealer quotes per feature gap
--   4. Dynamic insights via get_vendor_actionable_insights()
--   5. Used row_to_jsonb (not a real PG function)
--
-- This migration restores the 20260313200000 version (the most
-- complete) and merges in the sentiment enrichment additions
-- (NPS tiers + expanded sentiment counts in history).
