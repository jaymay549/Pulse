-- CAR-19: allow explicit comparison-category overrides in the leaderboard.
--
-- The frontend still defaults to automatic category selection, but users can
-- now choose any category where their vendor has mention history.
...
BEGIN
  ...
  SELECT array_agg(category ORDER BY is_other ASC, mention_count DESC, category ASC)
  INTO v_available_categories
  FROM (
    SELECT
      vm.category,
      (vm.category = 'other') AS is_other,
  ...
  ELSE
    IF p_category_override IS NOT NULL AND p_category_override <> '' THEN
      v_category := p_category_override;
    END IF;

    IF v_category IS NULL AND v_product_line_id IS NOT NULL THEN
  ...
  RETURN jsonb_build_object(
    ...
    'segment', jsonb_build_object(
      ...
      'available_categories',    COALESCE(v_available_categories, ARRAY[]::TEXT[]),
...
GRANT EXECUTE ON FUNCTION public.get_compared_vendors(TEXT, INTEGER, JSONB, TEXT, TEXT)
