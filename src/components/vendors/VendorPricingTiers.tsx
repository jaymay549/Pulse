...
export const VendorPricingTiers: React.FC<VendorPricingTiersProps> = ({
  ...
  const currentViewerPricing = isAnnual
    ? viewerPricing.annual
    : viewerPricing.monthly;
  const currentVerifiedPricing = isVerifiedAnnual
    ? verifiedPricing.annual
    : verifiedPricing.monthly;
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-gradient-to-r from-primary/5 via-yellow-500/10 to-primary/5 border-2 border-border">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto mb-8">
          ...
          <div className="p-5 pt-8 rounded-xl bg-white border-2 border-border relative flex flex-col min-h-0">
            ...
            <div className="flex items-center justify-between gap-2 mb-4">
              <div>
                <span className="text-2xl font-bold text-foreground">
                  {currentViewerPricing.price}
                </span>
                <span className="text-sm text-muted-foreground">
                  {currentViewerPricing.period}
                </span>
                {isAnnual && (
                  <p className="text-xs text-green-600">
                    {viewerPricing.annual.subtitle}
                  </p>
                )}
              </div>
            ...
            >
              <a
                href={currentViewerPricing.link}
                target="_blank"
          ...
          <div className="p-5 pt-8 rounded-xl bg-white border-2 border-primary/30 relative flex flex-col">
            ...
            <div className="flex items-center justify-between gap-2 mb-4">
              <div>
                <span className="text-2xl font-bold text-foreground">
                  {currentVerifiedPricing.price}
                </span>
                <span className="text-sm text-muted-foreground">
                  {currentVerifiedPricing.period}
                </span>
                {isVerifiedAnnual && (
                  <p className="text-xs text-green-600">
                    {verifiedPricing.annual.subtitle}
                  </p>
                )}
              </div>
              <div className="flex items-center gap-1.5">
            ...
            <ul className="space-y-2 text-sm text-muted-foreground mb-4 flex-1">
              ...
              <li className="flex items-center gap-2">
                ...
                <span>
                  <strong className="text-foreground">Company profile</strong>{" "}
                  page
                </span>
              </li>
              <li className="flex items-center gap-2">
                ...
                <span>
                  <strong className="text-foreground">Monthly analytics</strong>
                </span>
              </li>
            </ul>
            <Button
            ...
            >
              <a
                href={currentVerifiedPricing.link}
                target="_blank"
                rel="noopener noreferrer"
              >
