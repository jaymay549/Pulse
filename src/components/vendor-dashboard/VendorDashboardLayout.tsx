...
export function VendorDashboardLayout({ vendorName, activeSection, onNavigate, children, tier }: VendorDashboardLayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans antialiased text-slate-900">
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 h-12 bg-white px-4 flex items-center">
        <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
          <SheetTrigger asChild>
            <Button variant="ghost" size="icon" className="-ml-2 text-slate-500 hover:text-slate-700 hover:bg-slate-100">
      ...
      {/* Body */}
      <div className="flex flex-1 lg:pt-0 pt-12">
        {/* Desktop sidebar */}
        <div className="hidden lg:flex fixed top-0 bottom-0 w-60 z-30">
          <VendorDashboardSidebar vendorName={vendorName} activeSection={activeSection} onNavigate={handleNavigate} tier={tier} />
        </div>
